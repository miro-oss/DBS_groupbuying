...
export default function SellerManage() {
  ...
  const handleSelectAll = (e) => {
      if(e.target.checked) setSelectedIds(submissions.map(s=>s.submissionId));
      else setSelectedIds([]);
  };
        ...
        <table className="table" style={{background:'white', margin:0, minWidth:'800px'}}>
            <thead style={{background:'#f8f9fa'}}>
                <tr>
                    <th style={{width:'50px', textAlign:'center'}}>
                        <input type="checkbox" onChange={handleSelectAll} checked={submissions.length > 0 && selectedIds.length === submissions.length} />
                    </th>
                    <th>입금자명</th>
                    <th>연락처</th>
                    <th>수량</th>
            ...
            <tbody>
                {submissions.length === 0 ? (
                    ...
                    submissions.map(sub => (
                        <tr key={sub.submissionId} style={{background: selectedIds.includes(sub.submissionId) ? '#fff8f0' : 'white'}}>
                            <td style={{textAlign:'center'}}>
                                <input type="checkbox" checked={selectedIds.includes(sub.submissionId)} onChange={()=>toggleSelect(sub.submissionId)}/>
                            </td>
                            <td style={{fontWeight:'bold'}}>{sub.buyerName}</td>
                            ...
                            <td>
                                <span style={{
                                    ...
                                    borderRadius:'12px', 
                                    fontSize:'12px', 
                                    background: sub.paymentStatus === 'WAITING' ? '#fff3cd' : '#eee',
                                    color: sub.paymentStatus === 'WAITING' ? '#856404' : '#333',
                                    fontWeight:'bold'
                                }}>
